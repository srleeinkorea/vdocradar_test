...
import { Zap, AlertTriangle, CheckCircle, Search, ChevronDown } from 'lucide-react';
...
const App = () => {
                    ...
                    <thead className="bg-gray-700">
                        <tr>
                            {['ID', '사용자코드', '청구코드', '코드명', '청구항목', '구분', '처리방법'].map((header) => (
                                <th key={header} className="px-4 py-2 text-left text-xs font-medium text-gray-300 uppercase tracking-wider cursor-pointer hover:bg-gray-600 transition">
                                    <div className="flex items-center">{header} <ChevronDown size={12} className="ml-1" /></div>
                                </th>
                            ))}
                        </tr>
                    </thead>
    ...
    const ResultSection = () => (
        <div className="flex flex-col space-y-4 p-4 bg-gray-800 rounded-lg shadow-xl h-full">
            <h3 className="text-lg font-semibold text-gray-100">심사 내역</h3>
